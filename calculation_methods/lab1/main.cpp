...
//        std::cout << i + 1 << " iteration:\n";
    Matrix A = Matrix::get_random_symmetrical_matrix(5);
    Matrix x = Matrix::get_random_vector_column(5);
//    Matrix A = Matrix({{3, -2}, {5, 1}});
//    std::vector<std::vector<double>> vec{{-6}, {3}};
//    Matrix b = Matrix(std::move(vec));
    Matrix b = A * x;
    std::cout << x << "\n" << Matrix::solve_by_relaxation_method(A, b, 1.0 - 6.0 / 40, 1e-12);
//        timer.start();
...
//        std::cout << i << ": " << dur << " microseconds\n";
//        auto c = A.get_LUP_by_columns_decomposition();
//        timer.start();
//        Matrix::solve_LUP_by_columns(c, b);
//        dur = timer.get_time_in_microseconds();
//        std::cout << i << ": " << dur << " microseconds\n";
